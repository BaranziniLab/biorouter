 ...
 * ground with the 2px accent bar. A channel the person left an unsent draft in shows a pencil in
 * the count's place while it has no unread count (a count wins), and its name says ", draft"
 * (Q3-09); the channel on screen never does, its draft being in the composer.
 *
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <button
            ...
            data-draft={hasDraft ? 'true' : undefined}
            aria-label={`${sidebarCopy.channel.rowLabel(row.name, unread)}${hasDraft ? crewDraftCopy.rowSuffix : ''}`}
            tabIndex={tabIndex}
          ...
          >
            ...
            {unread > 0 ? (
              ...
              <Pencil
                ...
                aria-hidden="true"
                data-testid="crew-channel-draft-marker"
              />
            ) : null}
          </button>
        </ContextMenuTrigger>
