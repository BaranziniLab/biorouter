 ...
 *   pane closes.
 * - **Model**: the app's default model as a one-line summary with Change when it resolves to a
 *   configured provider, else the picker; "No models are set up." with Open Settings when nothing
 *   is configured (Crew bypasses provider onboarding).
 * - **Advanced** holds Also read (unmounted while closed, which keeps the unknown-outcome gate's
 *   checkbox the only one in the document, C9) and the remote folder line.
...
export function AgentTaskPane({ onShowTask, className }: AgentTaskPaneProps) {
  ...
  return (
    <form ref={form} onSubmit={onSubmit} className={cn('flex min-h-0 flex-1 flex-col', className)}>
      <div className="crew-pane-body flex flex-col gap-4 px-4 py-3">
        {unknown && (
          <section aria-labelledby={gateTitleId} className="flex flex-col gap-2">
            ...
            <div>
              <Button
                ...
                variant="secondary"
                size="sm"
                disabled={!crew.inspectedPriorRun || pending}
                onClick={() => {
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-element border border-border-emphasized bg-background-default px-2 py-1.5 text-body text-text-default transition-[color,background-color,border-color,box-shadow] placeholder:text-text-muted hover:inset-ring-2 hover:inset-ring-border-emphasized/30 focus:inset-ring-0"
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          {summary && selected ? (
            ...
            >
              ...
              <span className="flex min-w-0 flex-1 items-center gap-1.5 text-label text-text-default">
                <span className="min-w-0 truncate">
                  {selected.model} · {providerLabel(selectedProvider, selected.provider)}
                </span>
          ...
          <div className="flex flex-col gap-3">
            {otherChannels.length > 0 && (
              <fieldset className="flex flex-col gap-1">
                <legend className="mb-1 text-label text-text-default">{agentCopy.alsoRead}</legend>
      ...
      <div className="crew-pane-footer flex flex-col gap-2 px-4 py-3">
        ...
        <div>
          {showError && crew.error && (
            <Note tone="danger" role="alert" icon={AlertTriangle}>
              <p>{crew.error.message}</p>
            </Note>
          )}
        </div>
        <div className="flex justify-end">
          <Button key="start" type="submit" disabled={startDisabled}>
            {agentCopy.start}
          </Button>
        </div>
      </div>
