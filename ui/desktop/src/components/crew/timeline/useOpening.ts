...
/**
 * For a daemon that does not mark the end of the backlog (no `remaining` on
 * its frames): the live tail counts as arrived once it has stopped growing for
 * this long.
...
export interface OpeningInput {
  ...
  /**
   * The observer's own word on the live tail's backlog (`remaining`): true once
   * it has arrived, false while more is coming. Absent from a daemon that does
   * not say, and then the stream is timed instead.
   */
 ...
 * the live tail once the observer says its backlog is in (`backlogComplete`)
 * or `openingProgress` proves it complete. Only for a daemon that marks no end
 * of the backlog is the stream timed instead: arrived once it has stopped
 * growing (`OPENING_QUIET_MS`, or `OPENING_STALL_MS` while messages are
 ...
 * automatic mark-read, `aria-busy` coming off, and live arrivals rising in.
 * Without it the first streamed message stood in for the whole channel.
 */
export function useOpening({
  ...
  const complete =
    pageReady && (arrived === loadKey || progress === 'complete' || backlogComplete === true);
  if (complete && arrived !== loadKey) setArrived(loadKey);
  // The daemon counts the backlog down: its word decides, and no timer guesses.
  const timed = backlogComplete === undefined;

  useEffect(() => {
    if (!pageReady || complete || !timed) return;
  ...
  }, [pageReady, complete, timed, progress, loadKey, size, newestId]);
