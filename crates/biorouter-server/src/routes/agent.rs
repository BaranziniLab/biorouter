...
async fn get_callable_tool_count(
    ...
    //
    // ⚠ **The refusal is returned as the gate wrote it, not through this route's
    // `ErrorResponse`**, which is why the gate sits in a wrapper here exactly as
    // it does in `get_tools` beside it. One boundary has one body: every gated
    // route answers with the PLAIN-TEXT bytes `GET /sessions/{session_id}`
    // returns, so a caller cannot tell the routes apart by their envelopes. A
    // `?` into this route's JSON envelope is what
    // `every_route_that_names_a_private_chat_refuses_it_exactly_as_the_read_does`
    // measures, and it fails on the wrapping alone.
    if let Err(refusal) = crate::routes::session_reach::session_reach(
        state.session_manager(),
        &query.session_id,
        &headers,
