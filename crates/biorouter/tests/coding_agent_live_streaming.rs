...
async fn codex_streams_a_real_turn() {
    ...
    // which Codex no longer serves to a ChatGPT sign-in (it is still active on
    // the OpenAI API) — `codex exec -m gpt-5.4-mini` now fails `400 … not
    // supported when using Codex with a ChatGPT account`, so this test could
    // no longer pass. `gpt-5.6-luna` is its Codex-side replacement, and
    // unlike the GPT-6 models it needs no particular
