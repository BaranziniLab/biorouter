        ...
        sites: &[
            ...
            Site {
                ...
                what: "`POST /agent/resume`, `POST /agent/update_from_session`, and `POST \
                       ...
                       they name. On a daemon that holds no user-action key \
                       `authorize_agent_control` is ALSO the turn-control gate of \
                       `/agent/cancel` and the two continuation routes (SD-11), reached from \
                       `routes::reply`'s `authorize_turn_control` by name — so SD-11 added no \
                       call here; `/interrupt` keeps the proof on every daemon and reaches \
                       neither gate. \
                       ⚠ The count went 4 → 6, and neither addition is a second gate on an \
                       already-guarded route: `/agent/tools` and `callable_tool_count` each \
                       had NO gate at all. The latter was found by an SD-8 review of #260 \
                       answering any caller that could name a chat, through \
                       `get_or_create_agent`, which CREATES an agent for a session that has \
                       none — so the fix is the same reach gate, placed before the fetch for \
                       the reason `agent_add_extension` states at its own",
            },
            Site {
