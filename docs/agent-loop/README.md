...
| [Steering a running turn](steering-a-running-turn.md) | The contract behind adding a message to a turn that is already running: where a steer can wait and what it waits for (the loop phases), what every way a turn ends does with a steer it accepted, how the renderer owns a steer until the daemon answers and keeps its spinner tied to a live turn, and the connection budget that keeps a steer from queuing in the browser. Current; `fix/steer-always-lands`. |
