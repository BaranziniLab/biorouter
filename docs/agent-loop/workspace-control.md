...
> ⚠ **If the app is the PACKAGED build, export `BIOROUTER_RENDERER_ORIGIN=file://` in step 1 as well.** The daemon's WebSocket gates admit, beside its own origin, only the one renderer its launcher declared, and a packaged renderer's page is a `file:` URL whose `Origin` is the literal `file://`. Normally the app declares that itself when it spawns its own daemon; a daemon **you** started has heard nothing from it, so without this the several-conversations tab channel (`/ui/workspace`) is refused while everything over plain HTTP still works. `just debug-ui`'s renderer is vite's page instead, and `just debug-server` already declares that one, so the developer pairing needs nothing. The declaration is deliberately not implied by anything the daemon can observe: it used to be, and a `biorouter serve` daemon then admitted any local `.html` file's socket.
