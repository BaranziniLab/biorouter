...
A steer is removed from the queue only after its row is stored, so a Stop landing mid-write leaves it for the runner's settle rather than dropping it. Its message ID and timestamp are assigned at admission; a keyed steer uses the reserved `steer:` prefix. The runner drops an interrupted database write before settling the queue, and settlement updates an already committed matching row rather than inserting a second copy.

## The renderer's side

- **Ownership.** `ChatStreamController.steer` owns a steer until the daemon answers: a network error, a 5xx or no answer within 8 s is retried with backoff under one idempotency key (the daemon answers a repeated key `202` without queuing twice); a first-attempt `409`, `400` or `403` is a refusal, and a refused queued message goes back to its own queue slot. Retries retain `expected_turn_id`, checked atomically with admission so an old request cannot steer a successor. Receipts survive turn closure and agent eviction (the agent retains 256 and the daemon 4096). After a transport failure followed by refusal, the renderer checks the keyed stored row. If delivery still cannot be confirmed, it keeps the text at its original display position with an explicit retry control instead of automatically sending it again. These recovery rows never enter server writeback history.
