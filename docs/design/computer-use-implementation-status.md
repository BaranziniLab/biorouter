...
Updated 2026-09-18. Current implementation source is `b3d67d48`; the fixture-only workflow update is included with this checkpoint. This ledger tracks the full user-requested result. Source changes, passing unit tests, packaging, actual GUI behavior, and a pushed pull request are separate gates. An unchecked row remains required; unavailable platform evidence is not a pass.
...
| Native `screen_capture` in the same helper as the nine upstream tools | Current patch `41f367ebd113bcb6f4686f0c0cd0c9193a874d73c8b2bcbde43588d3ad43573e` corrects permission-owner guidance. All five native targets pass hosted CI. Windows/Linux real fixtures passed; macOS BioRouter browser capture/input passed before this guidance-only patch. No xcap extraction or fallback. |
...
| Stop/revoke/cancellation, exact process cleanup, bounded timeout, no mutation replay | Cleanup/cancellation protections implemented; focused late-result regression passed. Live Stop rejected subsequent native calls; the UI confirmation was delayed and later settled. A timed repeat on the final package remains open. |
...
| macOS signing/notarization/TCC identity and upgrade continuity | Local ARM64 outer app and both Mac helpers now pass Developer ID signature verification. Normal app startup works without a mock keychain. Screen Recording remains granted; the newly signed helper requires Accessibility identity refresh, currently awaiting local macOS authentication. Notarization, Intel app installation and minimum-OS behavior remain open. |
...
- Pushed source `b3d67d48` passes the full local project gate and production build. Hosted native and frontend checks pass; Rust/platform checks are still running. All applicable checks passed at the preceding `f28eaba2` revision.
- Real BioRouter native actions produced eight independent fixture events: page open, synthetic text entry, confirm click, three scroll positions, bottom confirmation and second-page navigation. Receipt: `target/computer-use/evidence/live-browser-and-stop-20260918.json`.
...
- Stop blocked two subsequent native calls. Its delayed display settled without a code change; network timing is enabled for the repeat. The cause of the earlier display delay remains unconfirmed.
- The final app contains the new permission guidance, verified production binaries and signed helper. Passive doctor reports Screen Recording true, Accessibility false, integrity verified, and no runtime override: `target/computer-use/evidence/guidance-final-app-doctor.json`.
- The user approved both OS permissions. Refreshing the signed helper's Accessibility entry reached a macOS Touch ID/password dialog. The agent cannot authenticate on the user's behalf; local-app and final-package GUI tests await this step.
- `test_depth=fixture_only` now renders only Computer Use, restricts actions to named synthetic fixtures and reports in the terminal. Actual CLI renders for fixture-only, selected-phase and full-suite paths pass. The required live self-test has not yet run.
- The earlier ZIP receipt describes an older artifact. Final archive regeneration waits for the remaining acceptance checks; no release or notarization is claimed.
