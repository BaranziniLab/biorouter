...
Updated 2026-09-18. This branch integrates main through `cfb881e0`. Pushed source is `ff8db15e`; the live Mac candidate uses patch `ec320c55`, with further cross-platform scrolling and strict argument validation being consolidated. Package acceptance and live-test follow-up remain in progress. This ledger tracks the full user-requested result. Source changes, passing unit tests, packaging, actual GUI behavior, and a pushed pull request are separate gates. An unchecked row remains required; unavailable platform evidence is not a pass.
...
| Native `screen_capture` in the same helper as the nine upstream tools | Native capture shares the helper with control. Exact-window/focus ownership, typed AX values and Windows real-button drag are implemented. Live scalar readback and Mac three-page wheel scrolling passed; strict Mac parser has 205 passing non-live tests. Final Windows/Linux scroll review corrections and final-patch hosted validation remain open. No xcap extraction or fallback. |
...
| Source reviewed and pull request pushed | Draft [PR #330](https://github.com/BaranziniLab/biorouter/pull/330) is pushed and reviewable. Source `ff8db15e` is pushed; the separately committed background fix and consolidated native corrections await the next push. Final hosted gates and artifact acceptance stay open; a draft PR does not complete the other gates. |
...
- Latest Windows real-fixture diagnostics measured 95 pixels after a 92-pixel request through UI Automation, correctly failing the independent assertion. Reviewed patch `bffba359f00d2a36d58d1b87717a5354a46ad2ad9d26d96fa0a0c3fdaef4dfda` now prefers measured native scrollbar control on the exact selected UIA owner before any mutation; it never jumps to another ancestor or replays a failed native action through UIA. Routing/no-replay contracts and negative controls pass. All five updated payloads build and verify; actual hosted Windows rerun remains required.
- Production CLI/daemon rebuild completed in 11m40s. The signed isolated Mac app now uses backend `365c28fe` and helper `86fc7f97`, retains both permissions and passes backend program-byte provenance checks. Luna is running the bounded fixture-only CLI workflow. A subsequent Windows-only patch does not change that Mac runtime source.
- Cross-process ownership integration passed with real production consent locks: contender denied, successor admitted after task release while owner remains alive, and recovery after owner death. A retained-lease negative control fails as intended, then restored test passes. Commit `85b60aa8`; logs `/tmp/biorouter-cu-process-ownership-{negative,restored}.log`.
- Live separate-chat grants are verified, but private-to-public native handoff remains unverified: the private Versa provider rejected the synthetic request before acquiring control. The unrelated model-dialog disclosure is not handoff evidence. Receipt `privacy-isolation-20260918.json` explicitly records this limit.

- A fresh packaged CLI `doctor --format json --no-update` reports Accessibility and Screen Recording enabled, native runtime ready, integrity verified and no development override. The isolated live app remains PID 80316; no further user approval is required for the authorized tests.
- The actual wheel fallback passed an exact three-page request through BioRouter in a public-model chat against an isolated, nonpersistent WebKit browser. Independent page events measured 797, 1594 and 2391 pixels for a 797-pixel viewport. The captured HTML target has no AX page action, confirming wheel dispatch. Receipt: `target/computer-use/evidence/live-webkit-three-page-wheel-ec320.json`. Chrome-specific retesting was blocked by the private provider before the action; this receipt does not claim that test passed.
- Mac strict numeric parsing passed 205 non-live tests and independent review. Supplied nulls, booleans, strings, nonfinite values and unsafe counts reject before dispatch; defaults apply only to absent arguments. Valid fractional pages remain supported. This newer scratch delta is not yet in the live app.
- All five native jobs and the aggregate gate passed at `ff8db15e`. The local full project gate passed. Standard hosted CI exposed a real output-collection race in the Mac background-process test. Commit `365c28fe` joins stdout/stderr readers before publishing completion, bounds inherited open pipes, and preserves cancellation without signaling an exited process. Independent review and 27 targeted tests passed; the final full project gate and hosted rerun remain pending. The original failed run remains a failure.
- Corrected full-package acceptance run `35388308517` has passed macOS ARM64 DMG and ZIP creation and installed-runtime verification; Both GNU production builds also passed; Windows ZIP and Linux package jobs then found genuine packaging defects: Node 24 rejects the npm.cmd launch and the Linux archive lacks the required node-pty native module. These paths are being corrected without weakening verification. Intel macOS remains building. Receipts: `target/package-acceptance-evidence/35388308517/darwin-arm64/`. These artifacts use pushed patch `7fd33dda`, not the newer live/scratch changes; hosted doctors verify integrity with OS permissions unavailable. The source includes bottom-up signing of otherwise unsigned Electron frameworks and cache preservation after failure. Final-payload artifact acceptance remains open.
- Luna completed TextEdit, Auto/Approve/SmartApprove and Stop acceptance on the unchanged signed live app. Root verified transcript tool outcomes and the exact saved line count of one. Receipts: `textedit-native-validation-20260918.json`, `approval-modes-20260918.json` and `stop-revocation-20260918.json` under `target/computer-use/evidence/`. All three modes required a fresh grant and then permitted state/capture without per-action approval; Stop blocked the seventh native call after six successes.
- Consolidated scratch patch `c2e6bed0` built and verified all five targets, including both Mac protocol checks. Independent review found Windows one-unit no-op acceptance and Linux short-line false-boundary defects; both are fixed and independently re-reviewed. Patch `86fc7f974ba2d22d23285acf4327e1e2f49769260fa8dabfdea4d1a4b593fb11` includes these fixes. Linux 20 unit tests and the rebuilt real fixture pass, including eight fractional/multi-page directional cases, invalid-argument/no-movement safeguards, mixed-length boundary rejection, and drag/text/capture. Receipts: `target/computer-use/evidence/biorouter-linux-scroll-review-fixture*`. All five target payloads built and verified, including both Mac protocol checks; 11 packaging regressions pass. Receipt: `target/computer-use/evidence/payloads-86fc7f97.json`. Hosted Windows fractional/horizontal fixture acceptance remains open.
