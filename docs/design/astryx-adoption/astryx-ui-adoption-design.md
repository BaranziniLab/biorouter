...
One recipe, everywhere: a full-bleed hairline; title `24/600`; optional description in `secondary` muted; **actions on their own line under the description**, primary first. This replaces the three placements in use (inline-with-title, a button row below the description, none) and the column-capped hairline in Settings, Skills and Knowledge. Settings additionally loses its double rule (header border + tab-strip border a few pixels apart).

⚠ **The action placement was reversed on 2026-09-07, by the operator.** This paragraph read "**actions right-aligned on the title row**, primary at 36px and everything else at 32px" until that date, and Chat history and the Scheduler were both built to it and cited it in their comments. The operator asked for the opposite — *"the orange button that says 'New Schedule' and the little refresh icon would appear on different lines, exactly the same way as the workflows, extensions, skills, and applications are built apps … I'm trying to make sure that they are following the same design principles"* — naming the button row below the description as the shape the rest of the app should match. So the recipe is amended here rather than contradicted in code, and the two views that had adopted the original were moved onto the amendment in the same pass.

There is an argument for it beyond consistency, and it is visible in the markup this change deleted: a title row that also carries controls has to give the title `min-w-0 truncate` so the pair yields in the right order, which makes a page title something its own buttons can clip. A title on its own line cannot be.

The recipe now has ONE implementation — [`components/Layout/PageHeader.tsx`](../../../ui/desktop/src/components/Layout/PageHeader.tsx) — used by Workflows, the Scheduler, Extensions, Skills, Built apps, MCP apps, Chat history and Settings. Its actions sit in `.biorouter-settings-control-strip`, the same strip Settings uses for a section's buttons, so a page's actions and a section's are one shape rather than two that nearly match. `styles/measures.test.ts` asserts at the source that each of those views imports it, so a ninth view cannot quietly grow a ninth copy of the header. The per-button pixel sizes the original sentence carried are the `Button` primitive's rungs and are not restated at the call site (vocabulary rule 7).
...
- **ScheduleDetailView** predates the flat redesign wholesale: no `MainPanelLayout`, `h-screen` sizing (the exact anti-pattern the layout's own comment warns breaks embedded panes), a muted ground, `label:` colon sentences instead of definition rows, and per-semantic tinted button variants that exist nowhere else. It is rebuilt on the standard scaffold with Astryx's **definition-row** pattern: 36px hairline rows, muted label and value left, a text action right.
  ...
  **Done, 2026-09-07**, together with the list and the create/edit dialog beside it. The view is `MainPanelLayout` → `ReadableContent size="chat"` with a §4.2 header whose title is the schedule itself; the facts are definition rows built from `.biorouter-settings-row` — the same row Settings uses for a label and the control it names, so a fact and a control are one shape rather than two near-misses of one — with a muted label left and the value on the trailing edge, in `font-mono` where it is a path, a cron expression or an id. The rows sit at `--row-height` (40px) rather than the 36px this bullet specifies, because A-03's retune is still a later phase and one view stepping to it early would be the drift the ladder exists to stop. The three tinted button variants collapse into one `.biorouter-settings-control-strip` on the standard ladder, the two loose warning sentences into one `<Note tone="warning">`, and the 3-column grid of run cards into a hairline list of rows sharing `ChatKindIcon` and the billed-token helper with Chat history. The list view lost its filled status pills for the dot-plus-word idiom (§3.4) and its button strip for §4.2's actions-on-the-title-row; the dialog moved onto `ModalShell` at the `md` form rung, with the cron picker as one inline sentence of `Select`s and no `<input type="number">` left in it. Both surfaces read the chat measure, like Settings — see `docs/desktop-ui/settings-visual-vocabulary.md`.
