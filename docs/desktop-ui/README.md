...
| [The Playwright end-to-end suite](e2e-playwright-suite.md) | The four gates `ui/desktop/tests/e2e` sits behind (ungated, LIVE, EXTERNAL, packaged), how to run each, and the sandbox contract a live spec must honour — `BIOROUTER_PATH_ROOT`, a private user-data dir, and an emptied `schedule.json`. Covers the trap that costs the most time (a startup modal makes `getByRole` fail for the whole page, because Playwright skips `aria-hidden` subtrees), why the packaged app cannot be launched by `electron.launch`, and an assessment of what CI would need — today no workflow runs `tests/e2e`, and neither `lint:check` nor `format:check` covers `tests/`. Current; measured 2026-09-09. |
...
| [The settings visual vocabulary](settings-visual-vocabulary.md) | The ten rules the Settings view (Models, Chat, App), the chat-history surfaces, the Scheduler and the component views (Workflows, Extensions, Skills, Built apps) are built to, and the primitives they lean on: a row's fill never depends on its state, rows are direct children of their list, one note shape with a ceiling on it, type roles rather than sizes, the button ladder, and one shared page header whose actions sit on their own line under the description. Covers why six of them are asserted at the source rather than in a render test — jsdom never runs Tailwind, and two of the defects only appear in the cascade. Current. |
