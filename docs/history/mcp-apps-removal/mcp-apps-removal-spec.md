...
**Where the removal record goes** (`docs/organization.md` §3, lines 144-150): create **`docs/history/mcp-apps-removal/`** with a `README.md` whose first paragraph gives the date, says the feature was present from the first commit, `54153b8b` (2026-01-20), was never requested, and was removed; and says where the current truth lives (`docs/desktop-ui/artifact-display-surfaces.md`). The exact precedent is `docs/history/dashboard-mode/` — README + `dashboard-mode-removal-spec.md` for a feature built and then deleted. Optionally add `mcp-apps-removal-spec.md` (this map). Then add the folder to `docs/history/README.md`'s index (§6: every folder carries a README, and history has an index).
