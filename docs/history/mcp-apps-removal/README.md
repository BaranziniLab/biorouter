...
- **`POST /agent/read_resource`.** After the removal it has no caller in this repository; its only one was the deleted renderer. It stays because it is a generic MCP capability rather than MCP-apps machinery, and an external client could legitimately call it — it is the resource sibling of `POST /agent/call_tool`, which was kept on the same reasoning, and `docs/deployment/programmatic-session-access.md` documents it as part of the programmatic HTTP surface. **The separate decision was taken on 2026-09-09: keep, and guard.** Losing its caller had left two defects nothing was exercising. It passed `None` as the admitted capability, which made Gate C's sibling sample the *named session's* bound model — so an HTTP client holding the daemon secret could name a private chat and read its private extensions' resources on that chat's reach. That is the "borrow another session's capability" hole the privacy execution plan closed at `call_tool` and ruled the same way here (§13012); only `call_tool` was migrated. And it collapsed every failure to `500`, so a privacy refusal was indistinguishable from a crash — the identical defect `dispatch_failure_response` had already been extracted to fix for `call_tool`. The route now declares `CallCapability::public_enforced()` like its sibling, and classifies: `403` for a refusal carrying its own words, `404` for an unknown extension, `502` for an extension that failed the read. `read_resource_route_tests` in `routes/agent.rs` pins both, and the capability census in `crates/biorouter/tests/privacy_capability.rs` now counts two production entries in that file rather than one.
...
`docs/desktop-ui/preview-panel/current-state.md` recorded `http:` in `frame-src` as load-bearing **for this proxy iframe**, which ran at the daemon's origin. With the proxy gone, whether that allowance could be dropped was left open — the claim had never been traced to the live policy, and the removal did not answer it.

**Answered on 2026-09-09: dropped, and so were the other two.** Both policies that govern the renderer document — the `<meta>` in `ui/desktop/index.html` and the header `ui/desktop/src/main.ts` attaches in `onHeadersReceived` — now read `frame-src 'self'`. `http:` really had been the proxy's; `blob:` and `https:` had never been traced to a frame at all. Every frame the renderer creates is `srcdoc`, the blob URLs in it are `<img>` sources, downloads and `window.open` — none of which `frame-src` governs — a PDF renders onto a canvas through pdf.js, and the live browser is a `WebContentsView`, a top-level context outside page CSP. `ui/desktop/src/frameSrcCsp.test.ts` pins both policies and scans every `<iframe>` the renderer writes, so the policy cannot outlive the fact that makes it right. The same edit covers `biorouter serve`'s browser bundle, which receives only the `index.html` meta policy — `routes/web_ui.rs` sets no CSP header of its own.
