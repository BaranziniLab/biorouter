...
| `GET /agent/computer_use/status` · `POST /agent/computer_use/revoke` | Whether the chat holds a live Biorouter Copilot grant, and the withdrawal of one. Approving a grant does not: see below. |
...
| `POST /agent/computer_use/consent` | `X-User-Action`, or `X-Computer-Use-Key` on a daemon started with a Biorouter Copilot approval key (`biorouter serve --computer-use-approval`). Letting a model see and control the desktop is the person's decision, so no capability header admits it; the route applies the reach gate as well, but the header alone is never enough. Ten failed attempts within sixty seconds lock the route for a minute. |
...
to a caller holding only the secret (M2), and both are now gated. `GET /agent/computer_use/setup` names no session at
all: it reports whether this host can run Biorouter Copilot, and is ungated.
