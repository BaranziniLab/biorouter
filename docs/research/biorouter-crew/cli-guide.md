...
| Stopped here, not confirmed (503 `crew_revocation_unconfirmed`) | `Stopped on this device. The workspace hasn't confirmed the revocation yet; Biorouter confirms it with the workspace by itself when the connection is back. biorouter crew grants list shows when it has.` | 1 |
...
After the 503 the grant is already expired and saved on this device, so this computer's chat can no longer use Crew, but the workspace has not yet confirmed that the run is revoked. The daemon asks the workspace again by itself: at every connect, whether a person's `connect`, a keepalive re-dial or a request that re-dials, and, while connected, at growing intervals until the workspace confirms. No second command is needed, though running `grants revoke` again asks at once. The daemon keeps asking even if the chat is granted access again in the meantime; `grants list` then shows the earlier run under its own heading. `grants list` shows the state: `Stopped on this device; the workspace hasn't confirmed yet` until then, and `Revoked` afterwards. With `--output-format json`, each grant carries `revocation` (`unconfirmed`, `confirmed` or `ended_by_workspace`) and `remote_revocation_confirmed`. With `--output-format json`, every failure prints `{"error": …, "request_id": …, "code": …}`, where `code` is the daemon's code, such as `crew_revocation_unconfirmed`.
...
A grant can also end at the workspace after access was granted: when the workspace's policy epoch moves (a policy change, or any membership change), when the run's context becomes protected, or when its institution changes. The next Crew request is then refused with `Crew settings changed since access was granted. Grant access again from Crew.`, the grant is stopped on this device, and `grants list` shows `Ended: Crew settings changed` (`revocation: ended_by_workspace`).
