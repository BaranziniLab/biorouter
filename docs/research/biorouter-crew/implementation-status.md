...
Earlier product `906bf68b` has passing pinned native/Linux builds and a full local gate; workflow/tests are `0fb6cd13`. Exact artifact hashes are below. The final committed `906bf68b` desktop suite passes 550 files: 6,256 tests passed, 19 skipped and zero failures (6,275 total), in 161.42 seconds Vitest/161.85 seconds wall time. The log is `/private/tmp/crew-ui-vitest-final-906.log`. Bounded pinned-current-artifact watch, file-mode/stale-capability and task-mode checks now pass; the complete matrix remains open.
...
[Draft PR #366](https://github.com/BaranziniLab/biorouter/pull/366) remains at hosted head `3145dfc5`, with 22 successes and one skip. Later product `8d6c2ae4`, transfer `3cbbf210`, observer `37f2684a`, stop fix `6c9c4ea6`, prior `906bf68b`, current transfer/transport commits `67a32a07`/`9a3957d6`, documentation `bd8c3970` and screenshots `b90939b7` remain local. Automatic review rejected screenshot export through `git push`; publication choice is pending. AWS product-source export and native CUA approvals are separately pending. No rejected action was retried through a workaround. Windows native qualification remains open despite source integration and cross-compilation.
...
Corrections for the two confirmed P2 transfer findings are committed in `67a32a07` with source-review closure and bounded focused passes; runtime rebuild/live qualification remain pending. The findings concern stale overwrite-target approval on Unix/Windows and Unix named-partial replacement after held-descriptor validation. Earlier `906bf68b` artifacts and passes do not validate these newer corrections.

The reviewed SSH correction committed in `9a3957d6` makes fatal transport errors persistently unusable and retires only the matching transport Arc to disconnected state with a safe error. It neither retries automatically nor shuts down the authentication master. Retirement after arbitrary caller cancellation is deferred until the next admitted request or explicit reconnect. Nine focused transport regressions, strict core Clippy and formatting pass; new builds, full gates and live confirmation remain pending. The [broken-pipe diagnosis](evidence/crew-cli-observer-context-20260922.md#broken-pipe-recovery-diagnosis) still has no established cause: normal reconnect reproduced the daemon history failure, while a separate direct bridge returned two hello responses and one challenge response (3/3). This is protocol isolation, not a recovery or backpressure pass.

The [authority/provider-counter API checks](evidence/crew-cli-observer-context-20260922.md#authority-and-provider-counter-api-checks) measured one public control request and private/public-provider refusal with the sink still at one. Missing/wrong human proof returned 403 with no negative-test receipt. Foreign-owner cancellation returned 400; owned cancellation returned confirmed revocation and one cancelled event, while the completion-first control stayed completed. These are bounded local API measurements, separate from graphical and broader task-mode cases.
...
| G13 | Shared daemon transfers preserve bounded memory, binary integrity and local file authority. | Interrupt/restart/resume, no duplicate publication, source/destination replacement, symlink/reparse, unauthorized overwrite and atomic finalization | partial | Earlier review and bounded transfer passes retain their scope. Test and replay the source-reviewed overwrite-target and named-partial replacement corrections, then complete the unblocked fault/resource matrix; retain native Save approval separately. |
