...
Published checkpoint `7e2ff404` retains the real-broker queued source-ACL acceptance and its explicitly scoped test authority; see [queued-source evidence](evidence/queued-source-acl-20260923.md). The compression correction in `commands/agent.rs` uses the real `response_compression` helper to exclude NDJSON from compression. Three actual production-helper tests pass: large valid JSON is gzip-compressed, open NDJSON delivers its first frame uncompressed, and open SSE delivers its first frame uncompressed. The ordinary daemon build and full local gate pass with no dependency diff. Focused review found no actionable ownership/privacy findings; authentication is unchanged. The new immutable pair is `/private/tmp/crew-gui-pair-20260923T0842Z`; the GUI lane used an authenticated stop for old daemon 51244 and installed the immutable daemon as PID 71261, instance `dd102a5d-e5e4-4877-9faa-e8c56f3a9c53`, preserving profile `8f70f965-a74a-4f22-b9d4-0b7662d625fd` and its runtime/socket; `user_action_installed` is true. Alice Electron is deliberately closed pending manual readiness and one attach prompt. No post-fix GUI pass is claimed.

Pre-fix observation: Alice native-home19 ran Electron PID 76672 with daemon PID 51244, proxy API port 63070 and exact connection `0cfdfb9e-7011-457c-9333-94ec0ebac9cb`. Playwright drove the actual Crew UI; Connect succeeded with the isolated synthetic SSH config and normal authenticated snapshot reads succeed. The app observer is HTTP 200 NDJSON with gzip encoding, but the renderer receives no initial state and remains on the first-connection screen. Renderer probes could not control Accept-Encoding because browsers forbid that header; their requested identity/gzip values are not evidence of actual identity encoding. A direct Unix-socket identity control returns HTTP 200 and decodes its first 623-byte state at about 2.5 seconds. The proper incremental gzip probe receives only 10 raw bytes at about 2.55 seconds and no decoded frame through ten seconds. This controlled comparison confirms compression buffering as the defect, not completion of a UI workflow.

Next acceptance priorities are manual post-fix GUI readiness and actual renderer first-state replay on the verified rebuilt pair, then three-user mixed GUI/CLI and native Save. The ordinary daemon now passes the [live HTTP retest](evidence/ndjson-compression-20260923.md): a gzip-requesting client receives an uncompressed initial state in 2.39 seconds while the stream remains open. Schema generation passes with no generated diff. Preserve published ACL evidence independently; no new full collaboration or cleanup pass is inferred. Earlier full-gate/native/Linux, 532c3b7d runtime and ae103eb5 hosted checkpoints retain their exact scopes in [validation](validation-report.md) and the artifact ledger.
...
Mixed GUI/CLI, native Save, native Windows and broader privacy/fault/recovery matrices remain open. The user explicitly approved native QA app control and verified-binary plus synthetic-data transfer to disposable AWS. The installed-build guard refused the packaged QA launch, while the supported stock-Electron development shell now starts and permits actual UI interaction through Playwright. Bounded AWS, queued-revocation and Alice setup interactions pass; full collaboration remains open. Shared IPC is Unix-only and standalone local conversations remain separate. All 26 visuals and the original-history backup remain local-only; source/text publication succeeded. Historical invalid fixtures, failed model attempts and unproven failure causes remain in the linked evidence, not upgraded by later bounded passes.
...
The apparent native SSH source bug is ruled out: fresh logs used the current short path, while the saved socket contained a 33-hex typo against the actual 32-hex path. A supported connection update fixed Alice’s connection. Prior 64-hex/control-master conclusions are excluded. The live queued source-ACL test now passes with clean profiles; see its scoped evidence above. AWS cleanup remains pending under absolute 09:45:00 UTC supervisor 40377.

Current compression artifact SHA-256 values are daemon `0890a909cd1e6933ae6aa06549275e5d0152a72c006b21a7bb713de0c7ee2cf6` and unchanged CLI `278db11f673b13ec866d9209a349610e1deda8f7800dc3eec36958b9b615ad3f`. AWS cleanup remains mandatory and unverified under absolute 2026-09-23 09:45:00 UTC supervisor 40377, replacing 84843.
...
| G09 | AWS instances, volumes, security groups, keys and fixture credentials are deleted and independently verified. | AWS cleanup report and post-cleanup resource queries | historical pass; current cleanup pending | The earlier fixture was independently cleaned at 13:40 UTC. The current approved AWS fixture is still live; persistent supervisor 40377 replaces 7908 and targets absolute 2026-09-23 09:45:00 UTC. Termination and absence of its volume/security group/keypair/local credentials must still be independently verified. |
...
Development apps were packaged and ad-hoc signed; the QA launch was refused by the intended installed-build development-profile guard, and no completed GUI readiness is established. Native QA app control and verified-binary/synthetic-data AWS transfer are now explicitly approved; execution is underway, and packaging is not GUI acceptance. Final bounded three-user attachment and natural `qwen3:8b` workflow now pass on `532c3b7d`; earlier `5455ebf9` evidence keeps its original scope.
...
- The AWS three-account fixture was prepared, but automatic approval
  ...
  AWS fixture are now explicitly approved; execution is pending, without a
  broader source-export claim.
...
- Current daemon/CLI and new Linux broker provenance are listed above. Hosted
  ...
  the linked reports.
- Native Save app control is now explicitly approved; actual execution remains pending.
