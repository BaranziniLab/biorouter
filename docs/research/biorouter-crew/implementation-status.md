...
Current source is pushed guard-only commit `ad161959` (parent `6c5a8640`); its CI is pending, and Rust/runtime artifacts plus running UI assets are unchanged. At 2026-09-23 16:19 UTC, all required PR checks passed on exact source checkpoint `6c5a8640fedf896277c3067d6c992e3e323e9a78`, including three OS Rust suites, Vitest, static/OpenAPI, cross/native and Linux packaging; cross-build-nightly was intentionally skipped. Package workflow 35840101914 verified GLIBC-floor and payload checks. Its ordinary x86-64 CLI/daemon/broker artifacts are mode 0555 with maximum imported GLIBC 2.30. These results qualify that checkpoint, not subsequent dirty source or complete GUI acceptance; [validation](validation-report.md) and the artifact table retain exact hashes and commands.
...
| G11 | Mixed GUI/CLI collaboration among three real Unix users shares identities, connections, messages, transfers and tasks. | Exact IDs/hashes/ownership, bidirectional visibility, policy and context outcomes | partial | Alice app/CLI instance identity matches and bidirectional message visibility passes with exact IDs/body. Bob and Carol attach and render Crew, but three-user collaboration, shared file/task/privacy and detach/reopen acceptance remain pending. |
