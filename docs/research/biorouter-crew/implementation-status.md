...
The Crew runtime checkpoint is `faefe7defaac43be4982408fbdbc3578082c7d19`; latest source also includes the narrow memory race correction. The deadline/projection correction passes independent review, 19 Crew route tests, one core preflight test, the full local gate and native build. Linux package workflow 35907167595 fully succeeds and macOS CI passes. Windows job 107337716983 failed the existing memory append/delete canonicalization race (1,643 MCP passes, one failure, three ignored); Ubuntu is still running. The memory lock-order correction passes independent review, 44 macOS focused tests and the full local gate. Windows-specific replay is pending; hosted CI is not yet green. Earlier bbcd CI and c496 package success retain their source scopes. Exact checks and hashes are in [validation](validation-report.md).

The current native pair is the deadline pair recorded below and in `/private/tmp/crew-deadline-native-handoff-manifest-20260923T190049Z.json`. All three profiles retain their saved connections. Alice’s GUI attach timed out at 19:08 UTC; Bob and Carol were not launched. There is no post-upgrade GUI acceptance. The latest self-test exited zero but failed required tool selection. A separate controlled follow-up advertised `crew__request`; that does not establish the original run’s metadata. The clarified test-only workflow passes five isolation tests, but its actual replay also exits zero without passing: a delegated child claimed refusal, supported export was privacy-gated, and no typed request-ID evidence was obtained. No bypass was attempted.

The prior AWS fixture was independently verified cleaned at 09:45:27 UTC. The current Ubuntu 24.04 x86-64 fixture uses distinct ordinary UIDs 10001/10002/10003, strict known hosts, caller-/32 SSH ingress, encrypted delete-on-termination storage and IMDSv2. Supported isolated CLI lifecycle passes on the exact 6c binaries, including stable profile/new instance and wrong-proof refusal. Cleanup is due at 20:30 UTC on 2026-09-23 under foreground session 5196/PID 68689; teardown remains unverified. The instance is running in us-west-2; exact final Linux transfer/execution is blocked by automatic review pending targeted user approval, not by instance unavailability.

Earlier qualified Electron artifacts passed three-client attach and Crew rendering, with three-client GUI message visibility and independent CLI ID/actor/channel matching. Alice GUI/CLI identity matches the same app daemon and bidirectional messages pass. Alice’s PNG upload, Preview and named native Save pass with matching bytes; installed manual Forget returns DELETE 200 and removes only the receipt. A fresh upload/post also removed its receipt without an alert, but no exact DELETE status was captured. Alice/Bob file visibility passes; Carol’s remains unqualified. Bob’s earlier PNG Save attempt was inconclusive; a subsequent binary Save passes. Authorized c9 non-owner removal returned HTTP 400 with owner-required refusal, and Alice independently confirmed all three memberships unchanged. G11 remains partial for the full file, agent/privacy and detach/reopen workflow.

One Alice owned read/write task has independent run/UID/output corroboration. Bob’s completed run failed its file/attachment objective. Carol’s distinct-input run reached the existing 15-minute execution limit without an artifact; its actual causal chain remains under investigation. Installed Bob Open agent session renders the persisted transcript, qwen3:8b, tool progress and final result without a global-default write or daemon restart. A new server summary-projection correction addresses a source-confirmed serial/lazy tool-poll deadlock pattern with bounded concurrent polling, active-exchange drain of at most 50 seconds, queued discard and non-fused EOF handling. Focused projection/preflight checks, the renamed cancellation test (1/1), schema checks, the full gate and native build pass after a pure helper extraction closed the initial Clippy length failure. The c9 artifact checks remain earlier evidence; the current deadline pair has no post-upgrade GUI pass. Supported native CLI repeated the identical private/public-provider request ID twice, receiving HTTP 400 with no new run or sink-count increase. Internal receipt absence was not directly read. A subsequent reviewed deadline correction keeps polling the cancelled future for 55 seconds after the outer 900-second limit so the internal 50-second exchange drain can finish; expiry remains primary, and unsettled cleanup is unconfirmed. That correction now passes 19 Crew route tests, one core preflight test, independent review, the full gate and native rebuild. All three stable profiles use the new immutable pair with 2/1/1 saved connections, currently disconnected; Alice’s attach timed out and Bob/Carol remain unlaunched, without a new GUI pass.
...
Carol’s final deadline-pair native owned task passes bounded read/write/attachment and verified 20-byte download. Remote execution, independent remote UID and tool-envelope IDs are not qualified. Leo permits source-supported chat/files but arbitrary execution is unsupported on Landlock ABI 1; Narrows NFS broker deployment remains blocked without a waiver.
...
|---|---|---|
| Current native CLI | Deadline pair, `/private/tmp/crew-deadline-native-artifacts-20260923T190049Z` | `f216e05b9d918f0bb50ac19cb632bb250b02774fdfea2ec5bff4fbd89647c04a`; full local gate/build pass; final GUI/runtime matrix pending. |
| Current native daemon | Same deadline pair and handoff manifest | `4af17abe9d233b559a368ab7a617a1f2ae6b0dd8064fcf01309046f6a059c6bf`; installed for all three stable profiles; Alice attach timed out, Bob/Carol not launched. |
