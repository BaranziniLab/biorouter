...
The implementation is published as [draft PR #366](https://github.com/BaranziniLab/biorouter/pull/366).
The last recorded pushed source is `3145dfc504ec51d92d299dfdc493669e61457bbd`, which
includes the FIOCLEX broker correction. Its Linux helper regression passes:
the exact CSV helper computes `row_count=3 total=60`, and network, subprocess,
other-ioctl and unauthorized-path negatives remain denied. The live three-user
fixture has been upgraded without resetting its workspace or history. Hosted
CI for `3145dfc5` is in progress; the last verified all-green hosted result is
22 active checks on `aac5f4ac` at 16:58 UTC, with nightly skipped as configured.

The earlier GUI macOS daemon/CLI evidence is from `aac5f4ac`, whose ten repository
checks, builds and version/help checks passed. Three isolated desktop clients
have verified reconnects, and four API public/private boundary scenarios pass
on that daemon with the earlier broker artifact. Human collaboration,
file transfer, owned-task controls and a tool-backed GUI CSV read have recorded
positives. Carol now has a [rendered GUI processing/attachment positive](crew-ui-acceptance-report.md#2026-09-22-carol-qwen38b-helper-processing-success): session `20260922_10`, private Ollama `qwen3:8b`, exact existing Python helper, exit 0, 21-byte output SHA-256 `29f857b262032224f20e849e76bed6907e8e02846835a632c8380b2bafb91ab3`, attachment `877d8a8e-e47c-41cd-9f9a-a03c99197f7c`. It used existing restricted `#processing-clean-2`; [independent backend and direct-SSH corroboration passes](local-provider-boundary-report.md#carol-processing-clean-2-successful-helper-corroboration-read-only), including retained typed calls, private provider/model, output owned by UID 1103 and daemon `1ce50cb3…`. **Successful personal-chat MCP, explicit cross-channel retrieval and completed user-named native Save remain open.** Native Save awaits exact signed-app CUA access approval.

Parity source/tests/generated API are now committed in `8d6c2ae4` (not pushed). Earlier committed source is `bc3e26fb` (SSH preflight) atop cursor/draft recovery commit `37ac3803`; the integration handoff records four broker cursor tests, six UI tests and typecheck passing. The [refreshed Linux artifact report](validation-report.md#refreshed-linux-broker-artifact-and-contracts-2026-09-22) records 30 passes, one ignored test and the helper/confinement regression for ELF `4f11d858…`, now installed in the three-user fixture, built before that source was committed. Native per-hop preflight is committed in `bc3e26fb` and independently source-reviewed; seven focused tests plus three affected admission/worker tests pass, as do formatting and whitespace checks. The legacy optional `ForkAfterAuthentication` test omits the config directive but modern `ssh -G` still emits that field; it is not absent-dump or old-client runtime evidence. Two separate pure missing-field unit tests now pass (2/2); old-client runtime remains untested. Combined build, applicable acceptance and final repository/hosted checks remain open. AWS product-source export remains blocked by automatic approval review; fixture resources were independently cleaned up at 13:40 UTC.

**The expanded completion scope is daemon-owned native CLI/GUI parity.** Shared services, native CLI and thin GUI adapters are committed in `8d6c2ae4` (not pushed); native/Linux builds now pass; integration reviews and complete live parity acceptance remain open. The existing broker/helper CLI, native SSH probes and GUI positives do not establish `biorouter crew` operation with Electron closed. No GUI-first percentage measures this scope. See [plan §15](implementation-plan.md#15-daemon-owned-workflows-and-cligui-parity), the ledger below and G10–G15.
...
SSH/MFA/jump support is core scope; the pending per-hop preflight must be
validated before claiming its security correction complete. Authentication
...
These are implementation states, not acceptance passes. `source present` below means the integration handoff records source committed in `8d6c2ae4` (not pushed); it does not mean final build, review, installation or runtime validation passed. Complete native CLI-only and mixed-interface workflows remain unaccepted; the isolated lifecycle subset now has passing evidence. The [source inventory](cli-parity-source-plan.md) is the original design inventory; use this ledger for current status.

Commit `8d6c2ae4` (not pushed) includes the product refactor, tests and generated API. It implements shared Unix daemon discovery/identity, Proven-only bootstrap and vault services, daemon PTY, streaming transfer capabilities/receipts, native Crew commands, grant/context/revoke APIs and thin GUI adapters. Identity is verified on the same connection before proof; descriptor structure alone does not authenticate an instance. Existing human proof remains independently held and is never persisted or replaced when attaching.

Native and Linux ARM64 CLI/daemon builds pass; the Linux artifacts are version `1.91.1` with measured GLIBC floor 2.39. The isolated bootstrap/approval/vault/stop/new-instance locked-restart flow passed. With all retained Crew Electron clients and their old owned daemons closed, three distinct users have passed native SSH, enrollment, team/channel creation, posting/history/search/read markers and unauthorized-channel refusal. Owned-task cancellation and remote-grant revocation are confirmed. Native MFA passes strict two-jump routing, an encrypted key and final PAM, with wrong-secret/cancel controls, retained connection after CLI exit and clean secret sweeps in curated reports. The clean three-user shared-blob download passes with exactly 43 bytes and matching hash; earlier locked-vault/disconnected failures remain recorded and do not prove a stale controller. Larger resume, agent and context cases are ongoing; full CLI-only, mixed-interface and recovery acceptance remain open. No result establishes that all remote jobs stopped. The actual `qwen3:8b` model self-test failed with reasoning-only output before Crew assertions; it is not a passing workflow.

The full desktop suite passes 547 files with 6,242 tests passed and 19 skipped in 181.8 seconds. Earlier focused UI runs overlap and are not added. Auth owned-child tests pass 5/5; CLI owner-lock tests pass 3/3 with focused Clippy passing. Current Rust Crew suites also pass: core `crew::` 37, server Crew 18 and CLI `commands::crew` 3. These scopes overlap earlier private/auth checks and are not added into a total. Rust filesystem checks pass 9/9 and private checks 4/4 after ACL/unconfirmed-restart fixes; native runtime 7/7, private/public vault 8/8 and 2/2 remain recorded separately. Source-review fixes for sidecar 5-second shutdown, 2-second join and 30-second CLI owner release are closed; runtime coverage remains bounded. The [curated 50-UID soak](evidence/crew-50-uid-soak-20260922.md) on broker `4f11d858…` records 30 minutes, 1,500/1,500 acknowledgments and exact readback/restart replay, with zero loss, hash mismatch or errors. It is broker evidence, not combined GUI/CLI responsiveness.

Generated OpenAPI is committed in `8d6c2ae4` after resume-schema and unique-operation-ID fixes. `just check-everything` passes end to end on `8d6c2ae4`: formatting/Clippy and TLS/socket checks across 17 targets, UI lint/typecheck/themes/404 contrast checks/tokens, generated OpenAPI drift, version/brand/naming/vendored cross-drift, registry 61 and privacy registry 21. This is the final local gate; the commit is not pushed and hosted checks for it are not established. The Windows helper is fully integrated in source and its real parent `local_files.rs` cross-compiles offline for `x86_64-pc-windows-gnu` on Rust 1.92; native Windows transfer/durability remains unqualified. Shared IPC remains Unix-only. Windows desktop functionality is required regardless of the unresolved optional Windows CLI scope. AWS source-export approval is pending, with no product AWS test. The separately rejected cross-task process/profile metadata handoff was resolved by retaining metadata in the authorized local test lane; it left no outstanding acceptance gate.
...
| P05 History/search/context/read/watch | Existing opaque broker queries plus new context endpoints; full shared-watch integration unverified | Historical polling/cursor evidence; new adapter unvalidated | History/search/read markers passed; watch/recovery still open | G10–G12: exact IDs, scoped cursors, source retrieval, watch/revocation/reconnect |
...
| P07 Upload/download/resume | Streaming transfer capabilities and durable receipts source present; independent review active | Thin React/native picker, overwrite confirmation and verified image preview source present; Windows helper integrated/unqualified native durability and named Save acceptance open | Clean three-user 43-byte shared download/hash passes; larger resume still open | G10/G11/G13: binary hashes, bounded memory, interruption/reselection, path identity and atomic overwrite rules |
...
| P09 Personal-chat grants and privacy | Grant/context/revoke endpoints source present; existing scoped privacy remains mandatory | Historical consent/API negatives; personal MCP/cross-channel success still open | Native grants/privacy command source present | G10–G12: same effective policy/context, revoke and zero forbidden sink bytes |
| P10 Shared typed API/client and packaging | CLI/server cargo check and bounded focused tests pass; generated OpenAPI committed; all local repository gates pass on `8d6c2ae4`; new hosted checks pending push | Shared proxy and thin adapters source present | Full command tree source present on Unix; shared Windows CLI unavailable | G15: matching commands/schema, exact final artifacts/checks and no duplicated business state machines |
...
| G05 | Real provider positive pass uses the actual BioRouter provider path and a public-safe fixture; negative pass uses a designated synthetic public sink and observes zero forbidden canary bytes. | Provider identity, sink digest/bytes, dispatch/tool/context traces | partial | Four API boundary cases passed on `6c19b87f…` and repeat successfully on current `aac5f4ac` daemon `1ce50cb3…`, with broker `7311f126…`: one allowed public control followed by zero additional requests/canary for workspace-private, personal-private, alias and retained-history refusals. The API calculated-result positive remains on `1ffa69d9…`; the backend-matched 31-byte GUI CSV read remains on `6c19b87f…`. The current daemon's three connections and API negatives do not establish its full model workflows. Bob's prior personal grant succeeded but MCP failed empty; Alice's latest cross-channel attempt returned stale CSV text without tools. Carol’s rendered GUI processing/attachment positive is recorded separately, with passing independent backend/direct-SSH corroboration; personal MCP and graphical privacy negatives remain open. |
...
| G07 | macOS, Windows and Linux client compatibility is measured separately for OpenSSH/MFA/process lifecycle. | Per-OS run records; no macOS substitution for other OSes | partial | Native CLI fixtures now cover forwarding-disabled approved exec, SFTP-only refusal, changed gateway/final-target keys and two manual PAM reconnects. These used no Crew artifact. The earlier short-path fix passed Electron PAM authentication/explicit close on `1ffa69d9…`, not a complete PAM Crew workspace connection. Desktop policy/reconnect cases, institutional MFA and independent Windows/Linux desktop acceptance remain open. New shared IPC is Unix-only. A Windows helper now exists but integration/durability acceptance remain open; required Windows desktop transfer functionality is not yet qualified. |
| G08 | All critical/high ownership, privacy, data-loss and provider-routing defects are fixed and replayed on one final revision. | Independent adversarial review, regression IDs, retest evidence | partial | `aac5f4ac` has all ten local checks, completed daemon/CLI builds, focused route/discovery/navigation tests and all 22 active hosted checks green. The FIOCLEX correction is pushed in `3145dfc5`, passes all ten local checks plus actual ARM64 Linux helper/confinement regression and pinned x86_64 bounded Bullseye smoke; its hosted pipeline is active. Cursor/draft work is committed in `37ac3803` with bounded focused passes; the refreshed Linux artifact passes its recorded contract/helper subset. Native per-hop preflight is committed in `bc3e26fb`, with seven focused and three affected admission/worker passes plus formatting/whitespace checks. Its legacy optional-field case does not establish old-client runtime support. The final local gate now passes on `8d6c2ae4`; new hosted checks and affected graphical workflows remain outstanding; no one-final-revision acceptance is claimed. Retained platform-client omission remains source-reviewed only. |
...
- Current daemon/CLI and new Linux broker provenance are listed above. Hosted
  green is established through `aac5f4ac`; pushed `3145dfc5` CI is in progress.
