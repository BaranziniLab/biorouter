...
Current source checkpoint is `533d4a7c80ab2e1dc4df9de4d9419e5db516e6b3`. All required hosted checks and exact-source Linux packaging pass; the full pre-push gate passes with supported Hermit activation. The current macOS native pair is installed across three retained profiles, with exact hashes and manifests below. All three macOS apps now automatically launch through the authorized synthetic stdin path without native prompts; the new three-user workflow remains underway. The latest full gate emitted every successful check and its final success message; a separate shell exit status was not captured. Exact public 533 Linux artifacts now pass a bounded fresh Alice daemon lifecycle on a newly approved fixture. The dev-only input path passes 46 focused tests, typecheck, isolated UI build and three automatic launches; it is later source than the 533 checks.

Recorded runtime subsets include three-client GUI messaging, GUI/CLI identity and bidirectional messages, PNG/binary native Save and receipt cleanup, owned read/write/attachment tasks, public-provider control/private refusals, and authorized non-owner refusal. Current 533 native replay returns two identical private-policy HTTP 400 refusals with no new run or provider dispatch. Carol’s faefe task has independent UID 10003 and Bob-download hash corroboration. Each pass retains its artifact and authority scope. The 533 upgrade was not wholly leak-free: its first Alice stop probe had terminal echo enabled and exposed an already-disclosed synthetic QA value to local tool output; later inputs used echo-disabled terminals. No value or raw output is published.

Bob’s requested remote calculation failed: the jobs attempted a missing script, invalid Python, and a string-literal no-op. Separate read/write/attachment bytes, UID and hashes pass, but no job computed the totals. Final independent diagnosis found no evidenced sandbox defect and no source fix warranted. The corrected-directory self-test used the prior faefe CLI, not 533, and remains NOT PASS: its refusal was model-reported without typed Crew evidence, followed by a malformed shell command. A later actual 533 self-test captured typed Crew `messages.history` refusal for no human-approved run, with empty `connection_id` despite the omission prompt; required help/version assertions were not executed, so it remains incomplete. Correct CR-input history inspection passes as a harness correction; neither result repairs the failed calculation.

The prior AWS fixture was independently verified cleaned at 20:33 UTC: exact instance terminated, volume/security group/key pair absent, and local key removal confirmed by the 20:30:26 receipt. This is distinct from earlier old-fixture checks; the proposed extension is no longer applicable. A new explicitly approved fixture under `/private/tmp/crew-three-user-20260923T2057Z-luna` is now active: all three users have hash-verified public 533 artifacts, and fresh Alice start/wrong-proof 403/restart-new-instance/stop passes. Its cleanup and broader execution qualification remain separate pending gates. Detailed IDs, commands, receipts, failed attempts and historical qualification are retained in [validation](validation-report.md) and the ledgers below.

Remaining completion gates are final-artifact GUI readiness/reopen and affected task/deadline/recovery replay, required self-test assertions, broader final Linux runtime, and the wider three-owner file/agent/privacy/platform/fault matrix. Shared IPC remains Unix-only; native Windows acceptance is incomplete. Leo arbitrary execution is unsupported on Landlock ABI 1, and Narrows NFS broker deployment remains blocked. Screenshots, secrets and visual-history backups remain local-only; bounded evidence does not close the full parity objective.

### Artifact provenance

| Artifact / evidence role | Source | SHA-256 / qualification |
|---|---|---|
| Current verified Linux x86-64 artifacts | `533d4a7c`, workflow 35911556727; `/private/tmp/crew-linux-533d4a7c-ci/verified/VERIFIED-MANIFEST.json` | CLI `bdeccf522d710f58f5cb71f32158964c8bbc289b65be007c41356633d2548c20`; daemon `9dd2c6e57b4076fc4a56f23e5904bc9c7bcd3cae8866aaca959237f550c24f8d`; unchanged broker `edc3133a8d80489b26497374d32472690179247b33394f0423ef7955fd81aac4`. Exact public artifacts hash-verified for three users; bounded fresh Alice daemon lifecycle/wrong-proof/restart/stop passes. No full Linux workflow or GUI qualification. |
| Current native CLI | `533d4a7c`, memory artifact manifest | `015a11e7b32ffd717dc212c63e11dc9d9631b4b0c19dd2173892babd9e8169b8`; macOS arm64 dev 1.91.1, mode 0555; build/current bounded native checks pass, final GUI pending. |
| Current native daemon | Same 533 artifact and completed upgrade manifest | `979c181c7ddc605d2c541eadb949a64bc9493b51232d9e33596a0bf5b014990c`; installed for all three profiles, no final GUI qualification. |
...
|---|---|---|---|---|
| G01 | Actual BioRouter Electron development app is built from this worktree and three independent app clients drive the visible Crew UI. | Build command, revision, sanitized screenshots/video, action timeline | pass at recorded revision | Three-client visible GUI workflows passed on earlier recorded artifacts, including shared-daemon messaging and Save. Historical aac5f4ac/1ce50cb3 evidence remains in validation. Current 533 native upgrade is complete but final GUI attach/reopen is unqualified; earlier passes are not reassigned. |
| G02 | Three real Unix accounts use distinct UIDs, home directories, SSH identities and profiles; no shared key or copied provider credential. | Disposable AWS fixture report with account/key/UID mapping | partial; recorded AWS three-user setup passes | The now-cleaned AWS fixture had distinct UIDs 10001/10002/10003, isolated profiles/keys, supported CLI lifecycle and earlier three-client messaging evidence. Prior fixture cleanup at 09:45:27 UTC is historical; the latest fixture cleanup is independently verified at 20:33 UTC. |
...
| G04 | Alice, Bob and Carol complete connect/enroll/general/analysis/files/owned-agent/context/revocation/recovery workflow with human chat before model selection. | Three-app evidence bundle and event IDs | partial | Recorded GUI messaging, invitations, native Save and Alice owned-task subsets pass. Carol faefe native read/write/attachment now has independent UID and Bob-download corroboration; Bob 533 read/write/attachment evidence passes, but typed job audit confirms remote calculation failed; the second script was invalid Python; no sandbox defect is established. Complete the final three-owner GUI/context/revocation/recovery workflow; earlier failures remain in validation. |
...
| G09 | AWS instances, volumes, security groups, keys and fixture credentials are deleted and independently verified. | AWS cleanup report and post-cleanup resource queries | prior fixtures verified; new fixture pending | Previous exact fixture cleanup verified at 20:33 UTC. New approved 2057Z fixture is active; its complete teardown requires a separate receipt. |
...
| G15 | One final revision delivers shared typed services and complete parity evidence. | CLI commands/help, generated schemas, exact binaries/commit, required checks, independent reviews and G10–G14 traces | not-run | 533 required hosted checks and Linux packaging pass; native upgrade is complete. Final GUI, self-test assertions and remaining G10–G14 runtime cases are not complete. Historical green checks do not substitute for those gates. |
