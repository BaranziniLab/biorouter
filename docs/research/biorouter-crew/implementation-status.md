...
Published source/text head `5455ebf90f8cd9d6e837d296820f6303a6a7cb66` is on draft [PR #366](https://github.com/BaranziniLab/biorouter/pull/366), titled “Add BioRouter Crew with shared daemon services and native CLI”. Its full `just check-everything` gate and immutable native release-pair build pass. Recorded bounded suites pass extension-install 31 and action-required routes 16; the preceding desktop suite passes 551 files/6,262 tests/19 skips. Current local HEAD `3dac36952ff4c822713bed3add9ba4f20005a64c` integrates `origin/main` at `06c9639a` and has independent source review with no findings. API regeneration produces the expected single export; merged-source checks and binaries remain unqualified pending reported results. Native Stop-and-Send takeover/abandon/leave is being implemented through the existing Proven/exact-generation daemon API, without automatic resubmission or lease exposure; tests are pending. This work is separate from committed `3dac3695` and verified `5455ebf9`. Source review or history equivalence does not requalify artifacts.

The immutable native `5455ebf9` pair passes shared-CLI create-only → exact-session grant → streamed turn with a deterministic private provider and actual `crew__connections`, history, context-manifest and run-project tool results. After exact revocation, the next turn receives typed refusal before provider dispatch (zero additional audited requests). A separate installed Ollama `qwen3:1.7b` plain-marker turn reaches Finish; it does not qualify natural Crew tool behavior. Natural `qwen3:8b` Crew use and observer faults remain pending; the forthcoming [shared-daemon evidence](evidence/shared-daemon-acceptance-20260922.md) records this bounded scope. The [Linux non-test production debug pair and UID 1101 lifecycle](evidence/linux-5455ebf9-20260922.md) also pass; copies are mode 0555 with unchanged hashes. Earlier `4a2e190b` production-pair lifecycle, three-user human roundtrip and `9a3957d6` transfer confirmation/replay/restart/cleanup retain their original scopes. [Validation](validation-report.md) and [CLI evidence](evidence/crew-cli-observer-context-20260922.md) preserve exact hashes, failed/invalid attempts, fixture corrections and bounded security results. The original server-hang and historical broken-pipe causes remain unproven; passing later checks do not establish causation.
...
|---|---|---|
| Current validated native CLI | `5455ebf9` | `b329b6ad161e9b6722ef6b08ebef9aab3df4462c25c516a3206f051525fc7bdc`; arm64 Mach-O 1.91.1, mode 0555; full gate/release build and bounded deterministic tool/revocation acceptance pass; natural Crew model and broader cases pending. |
