...
- **Verification.** The daemon requires at least one signature and verifies every one present; a missing or malformed field of a present v2 signature fails the connection. Only a verified v2 makes the name, mode, institution, policy epoch and capabilities trusted, and then for display only. A relay that strips `signature_v2` downgrades those fields to unauthenticated hints, which the daemon withholds; it cannot alter them.
- **Capabilities** are `human_chat`, `signed_devices`, `resumable_blobs`, `scoped_runs`, `human_names_v1` (the S1a projections and `expected_username`), `unique_names_v1` (S2a unique names and renames) and, only from a broker built with the `join-by-name` feature, `join_by_name_v1`. They describe the broker, not the workspace's identity: the daemon keeps them in memory per connection and never stores them in the saved connection (D12). A client sends a new parameter only after the capability that introduced it is present.
...
`invitation.create`, `membership.revoke`, `channel.transfer` and `enrollment.revoke` accept an optional `expected_username`: the `@username` the person confirmed when naming the target. It is part of the signed request, so it cannot be altered on the bridge path, and the broker refuses `target_mismatch` when the target principal's username differs. Clients send it whenever the person named the target by username.
...
- **Why `W` is inside.** An invitation tampered in transit to carry another workspace key yields a code the real broker never accepts, so it cannot admit anyone. This is defence in depth only; the two-sided code that discovery would need is deferred with discovery itself (D10, slice S4).
