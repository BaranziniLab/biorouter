...
Current source `532c3b7d` commits reviewed native continuation recovery and observer backlog corrections. Valid fresh-process CLI 11, daemon-client 16 and observer 12 regressions pass, and the full gate/native non-test build pass. Linux build/lifecycle, actual PTY leave/abandon/takeover and bounded 60-ID backlog/fairness/slot replay now pass. Published `c857cc1b` follows `532c3b7d` shipping code with docs/lock repair and a Windows test-only fix; completed Rust CI has 2 Ubuntu, 2 macOS and 5 Windows failures, plus seven Linux Vitest fixture failures. Source-audit, sanitizer/auth-rename and Windows fixture fixes pass local focused checks and the full gate; hosted rerun awaits commit, so no hosted-green claim is made. Final bounded three-user file and natural `qwen3:8b` replay now passes on `532c3b7d`; mixed GUI/CLI, broader fault/privacy coverage and native Windows acceptance remain open. All visuals stay local; AWS product-transfer and native CUA approvals remain pending. [Evidence](evidence/shared-daemon-acceptance-20260922.md) preserves bounded results and excluded fixtures.
