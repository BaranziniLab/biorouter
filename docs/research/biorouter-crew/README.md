...
Current product HEAD is `906bf68b` on `codex/biorouter-crew`. Native and immutable Linux ARM64 CLI/daemon builds and the full local gate pass. The final `906bf68b` desktop suite passes 550 files: 6,256 tests passed, 19 skipped and zero failures. Bounded live watch, adaptive history, admission limits and file/task privacy refusals pass on the pinned `906bf68b` pair; broader acceptance remains open. New transfer and fatal-SSH working-tree corrections have source-review closure but await tests; `906bf68b` passes do not cover them. Earlier matched artifacts have bounded CLI collaboration, MFA, transfer/resume, helper, context, personal MCP and privacy passes; they do not complete current-artifact or mixed-GUI acceptance. See [current status](implementation-status.md), [validation](validation-report.md) and [CLI evidence](evidence/crew-cli-observer-context-20260922.md).
