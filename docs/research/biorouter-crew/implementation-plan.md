...
| Ordinary terminal conversations | Reuse daemon agent/reply/session services and scoped grants; never forward human proof to local workers | Explicit shared-daemon deterministic and natural-model Crew tool/revocation cases pass on `5455ebf9`. Standalone local mode remains separate. Uncommitted native continuation recovery needs valid regressions and new-artifact replay; broader runtime and mixed-GUI matrix remain open. |
