...
Daemon-owned authentication, credentials, transfers, tasks, room observation and explicit shared conversations are implemented. The observer compression correction has actual Alice state-rendering evidence; Crew runtime source checkpoint is `faefe7defaac43be4982408fbdbc3578082c7d19`, with a later narrow memory race correction, including the bodyless-header correction; its empty-body JSON cleanup fixes pass focused tests and review; full gate/native rebuild pass; the corrected relative-assets renderer is installed and all three clients reloaded. Actual manual GUI Forget passes (GET 200, DELETE 200, forgotten true, receipt removed and attachment retained); a fresh upload/post removed its receipt automatically without an alert, with the exact DELETE status not captured. Exact parent `6c5a8640` passes all required hosted checks and Linux packaging. Rust/runtime artifacts and running UI assets are unchanged by the guard commit; local guard checks do not establish its runtime acceptance. The [implementation status](implementation-status.md) is the current revision/artifact/gate ledger.

Bounded acceptance now covers Alice GUI/CLI same-instance identity and bidirectional messages, three attached Crew views, supported ordinary-user AWS CLI lifecycle, and Alice’s actual PNG upload/Preview/named Save with matching hash. Ordinary Forget exposed a retained-receipt HTTP 400 JSON-EOF failure; the installed correction now passes manual Forget and observed fresh-upload receipt cleanup. Bob/Carol native enrollment/chat and actual GUI posts now have three-client visibility plus independent CLI ID/actor/channel matching; full G11 remains partial. Earlier natural-model/file/observer/PTY passes retain their recorded artifact scopes, while the [queued-source ACL evidence](evidence/queued-source-acl-20260923.md) and [compression evidence](evidence/ndjson-compression-20260923.md) record their specific authority and transport boundaries. Exact commands, hashes, failed setups and superseded diagnoses belong in [validation](validation-report.md), not this design summary.

Complete three-user mixed GUI/CLI collaboration, broader Save and cleanup behavior, agent/privacy and detach/reopen remain open, alongside native Windows and the broader provider/storage-fault/resource/recovery matrix. Shared IPC remains Unix-only and standalone local conversations remain separate. Approved AWS/native QA execution is underway; current AWS teardown still requires independent verification. Approval and bounded passes do not close the full completion gates below. No requirement permits weakening Proven-only human authority, publishing local-only visual/secret evidence or reassigning earlier binary evidence to a later source revision.

One Alice read/write task is independently corroborated; Bob’s completed run failed its file/attachment objective, and Carol’s run reached its execution limit without an artifact. The committed resume-session fix passes local checks and installed Bob session-resume verification. A source-confirmed serial/lazy tool-poll deadlock pattern is being corrected through bounded concurrent summary projection, cancellation-safe exchange drain and EOF handling; focused checks, the full gate and native build now pass; the native pair and Electron assets are installed, with post-upgrade GUI qualification pending. A subsequent reviewed deadline correction retains the cancelled future for bounded drain and rechecks cancellation before publication; its independent review, 19 route tests, one core preflight test, full gate and native rebuild pass. The new pair is installed in all three stable profiles; Alice GUI attach timed out at 19:08 UTC and Bob/Carol were not launched; final GUI acceptance remains pending. Epoch 3 provides an actual public assistant control followed by a fresh Private Crew refusal with no additional provider dispatch. Bob’s binary native Save passes; the two observed posts were separate deliberate submissions, with no automatic-retry or idempotency qualification. Earlier sink epochs and same-fingerprint replay retain their narrower limits in validation. The verified AWS cleanup deadline is 20:30 UTC, with teardown pending.

The latest self-test did not satisfy tool-selection assertions despite exit 0; five test-only isolation checks pass, but the clarified model replay also lacks typed request-ID evidence and is not a pass. Linux packaging and macOS CI pass; Windows hit the existing memory canonicalization race and Ubuntu is running. The reviewed memory lock-order correction passes 44 macOS tests and the full local gate; Windows replay is pending and the native runtime pair remains faefe. Final Linux runtime transfer/execution awaits targeted approval after automatic review refusal; the AWS instance remains available. Carol’s final headless task passes bounded read/write/attachment and verified download, without remote-execution or independent remote-UID qualification. Earlier c9 non-owner refusal and unchanged-membership readback are authorized bounded passes, not outstanding permission blockers.
