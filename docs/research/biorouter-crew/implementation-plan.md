...
One Alice read/write task is independently corroborated; Bob’s completed run failed its file/attachment objective, and Carol’s distinct-input run is pending. The reviewed resume-session GUI fix passes focused checks but passes its full gate/build and installed Bob session-resume check without changing the global default or restarting the daemon. Current hosted Rust/cross-Windows and Linux-package results remain pending; the verified cleanup deadline is 20:30 UTC, with teardown pending.
