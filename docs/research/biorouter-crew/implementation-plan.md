...
for this plan. Current production HEAD is `4a2e190b`, after transfer checkpoint `67a32a07`; earlier validated artifacts are
...
Current production HEAD is `4a2e190b`, adding independently reviewed safe transport diagnostics after `9a3957d6` transport retirement. Twelve transport tests, the full core Crew subset of 51 tests, strict core Clippy, formatting and diff checks pass. The full local gate and native CLI/daemon build pass on `4a2e190b` (2m02s). The immutable arm64 Mach-O 1.91.1 pair has mode 0555. The immutable Linux `4a2e190b` pair and focused filesystem 8, transfer 11 and proof 3 checks pass; the verified production-pair UID 1101 lifecycle rerun passes. The earlier test-feature daemon smoke is explicitly unqualified; see [artifact provenance](validation-report.md#verified-pair-uid-1101-lifecycle-rerun). These results do not cover the uncommitted conversation adapter; test scopes are not added together. The 550-file/6,260-passed/19-skipped desktop suite covers `d3498ecd`/`9a3957d6`; it does not cover the new uncommitted GUI elicitation changes. This follows transfer/GUI/API checkpoint `67a32a07` (14 files) and documentation `808e891f`. Transfer source review is closed. Focused passes for committed transfer source are transfer authority 6, transfer unit 11, route proof 3 and filesystem 9, plus strict server Clippy, API generation and formatting. The actual GUI-handler regression passes seven tests with ESLint and Prettier. These scopes overlap and are not aggregated. The independently reviewed transport correction is committed in `9a3957d6`; nine focused transport regressions, strict core Clippy and formatting pass. Native CLI/daemon builds (2m18s, version 1.91.1) and the full local gate now pass on product `9a3957d6` with docs `d3498ecd`. The gate covers 17 production targets, UI/schema/version/brand/naming/vendor/cross-drift checks, registry 61 and privacy 21. The current full desktop suite passes 550 files, 6,260 tests with 19 skipped and zero failures (6,279 total), in 160.06 seconds Vitest/160.46 seconds wall time; fresh diagnostic-artifact validation and broader Linux and native Windows qualification remain pending; bounded download confirmation is recorded separately. The original broken-pipe root cause is still unresolved. Fresh `9a3957d6` connection refusal was resolved as fixture setup: the restarted owned Alice daemon lacked its intended profile SSH environment; host keys matched. Restarting only that daemon with the correct environment produced native auth exit 0/`authenticated:true`, small history in 0.036 seconds, expected oversized-history `response_too_large` in 0.065 seconds and subsequent small history in 0.031 seconds. This is fixture recovery, not a product fix or an explanation of the historical broken pipe. Live upload/source-change checks retain their separate scope. Manager-backed download pending-confirm/replay now has bounded passes as recorded below; post-confirm replacement, download restart/resume and actual-partial cleanup also pass in bounded live checks; observer slow-reader/fairness and the broader fault matrix remain open. Safe diagnostics are reviewed and committed in `4a2e190b`, with 12 transport and 51 core Crew tests plus strict Clippy/formatting passing; the full local gate/native build now pass on `4a2e190b`; Linux verified production-pair UID 1101 lifecycle validation also passes; the earlier test-feature daemon smoke is excluded. The successful separate SSH probe omitted daemon ControlPath/profile configuration/known-hosts arguments, so it only proves separate-path broker responsiveness. Same-master comparison and fresh diagnostic-runtime validation remain pending, with no root-cause or historical-fix claim. The current native build/full gate does not qualify mixed GUI or native Windows. Manager-backed live download confirmation now passes on pinned `9a3957d6`, whose transfer source `67a32a07` matches current `4a2e190b`: pending registration/same-capability confirmation and completed hash match; changed-existing and absent-then-created targets both return 400 with sentinels preserved; missing/wrong confirmation proof returns 403; fresh receipt-bound replay returns 200 with the original receipt ID and unchanged destination hash. This closes that bounded confirmation gap. Bounded post-confirm replacement, download restart/resume and actual-partial cleanup pass on `9a3957d6` with the same transfer source as `4a2e190b`; broader faults, actual GUI and native Windows remain open; exact sanitized evidence is in the CLI report.

The validated `906bf68b` checkpoint contains the shared daemon services, thin native CLI/GUI adapters, transfer correction, room observer, stop-race correction and privacy/recovery guards. The daemon verifies connection identity and authoritative mode. GUI message/task/grant/file-selection requests and optional CLI `--expected-mode` bind the expected mode without changing the saved setting. Private-origin restrictions remain independent; cleanup remains receipt-bound. Explicit GUI Retry refreshes metadata before observing and uses abort/generation guards.

The shared `POST /crew/connections/{id}/observe` stream emits typed NDJSON State/Messages/Reconnect/Error events. The daemon reduces pages from 200 toward 1 only for `response_too_large`, uses opaque message cursors and revalidates source ACLs before buffered release. State updates continue every two seconds during backlog. Bounds are one queued producer item, five-second backpressure, 16 admitted observers including draining observers, and a 600-second admission lifetime. Active broker exchanges drain on detach to preserve shared SSH framing; mutex/exchange drain can extend teardown. Clients accept at most 1 MiB per frame including LF and reconnect only on typed events. Transient disconnect or `stale_cursor` preserves the bound draft pending refreshed history/ACL evidence; authoritative revocation/privacy changes clear it with an explanation.

Native and immutable Linux ARM64 CLI/daemon builds and the full local gate pass on `906bf68b`. Linux version/help, expected-mode help and disposable UID-1101 lifecycle checks pass with GLIBC floor 2.39. The final committed `906bf68b` desktop suite passes 550 files: 6,256 tests passed, 19 skipped and zero failures (6,275 total), in 161.42 seconds Vitest/161.85 seconds wall time. The log is `/private/tmp/crew-ui-vitest-final-906.log`. Current focused and bounded pinned-`906bf68b` live watch/file/task checks pass. The status ledger records observed refusals separately from provider nondispatch inferred from guard order. [Status](implementation-status.md) records exact hashes/counts; [validation](validation-report.md) preserves the historical sequence.

Earlier matched artifacts have bounded CLI lifecycle/MFA, collaboration, resume, helper, context, personal MCP, revocation and API privacy passes. The pinned `906bf68b` pair also passes live watch recovery, file expected-mode and exact stale-capability refusal with no receipt, plus task expected-mode refusal; provider nondispatch is inferred from guard order, not separately counted. The latest mismatched CLI/daemon watch attempt is invalid for product conclusions. The original concurrent-post/pause and initial context failures remain unexplained; five bounded model self-tests did not execute required assertions and are not passes; the final pinned-path attempt still selected a nonexistent MCP HTTP endpoint. Workflow/test commit `0fb6cd13` passes isolation checks without changing product `906bf68b`. A fresh independent fixture has executed CLI cases; old profiles remain preserved. GUI main/preload/renderer preparation builds and typecheck pass for product `906bf68b` plus workflow `0fb6cd13`, without launch, profile changes or CUA retry. Actual GUI interaction remains unqualified pending native approval. Mixed GUI, graphical privacy and native Windows acceptance remain open.
...
Two independently confirmed P2 findings affected stale overwrite-target approval on Unix and Windows, and Unix named-partial replacement after held-descriptor validation. Corrections are committed in `67a32a07` with independent source-review closure. Transfer authority 6, transfer unit 11, route proof 3, filesystem 9 and GUI-handler 7 checks pass as separate scopes; native `9a3957d6` artifacts and full gate pass; runtime replay remains pending. Earlier `906bf68b` checks do not validate these corrections.

Bounded independent API checks now measure one allowed public-provider request followed by private-mode refusal with the sink count unchanged, missing/wrong human proof refused with no new receipt, and foreign-owner cancellation refused alongside authorized controls. These do not close the full interface/privacy matrix. Unblocked local work remains on refreshed-artifact/runtime validation of the tested transfer corrections, broader restart/replacement faults and observer backpressure/fairness. An earlier observer attempt stopped before its assertions on a broken SSH request stream. The stale-connected metadata defect is corrected in `9a3957d6`, with independent review, nine transport regressions and strict core Clippy/formatting passing. Fresh runtime replay remains pending; the original broken-pipe root cause is unresolved. These requirements are not suspended by the external GUI, publication or AWS approvals.

A confirmed ordinary-terminal conversation gap remains: `biorouter run` and `biorouter session` create a local Agent whose CrewClient uses the process-local `crew::manager`. That manager loads metadata once, resets connection state, has no live daemon transports and keeps a separately locked vault. A daemon-issued grant therefore cannot supply the daemon's SSH connection: an existing grant reaches disconnected state, and a later grant is absent locally. Native `biorouter crew` human commands and owned tasks already use the daemon and retain their bounded passes. Earlier daemon-backed personal-session MCP evidence does not qualify these ordinary local-Agent entry points.

Required implementation is a daemon-hosted native conversation adapter reusing the GUI agent/reply/session services and the same scoped grants. The explicit shared-daemon adapter is applied in uncommitted source; all four adapter review findings and durable elicitation receipt review are closed in source; tests and runtime acceptance remain incomplete. Do not forward human proof to local workers, duplicate transport/vault ownership or weaken the Proven-only gate. Ordinary terminal personal-conversation MCP acceptance must execute through the resulting shared service before P09 can close.

The chosen ordinary-terminal adapter is explicit `--shared-daemon` on `run`/`session`, selected before local session, provider, Agent or project-bridge creation. `--create-only` obtains a real daemon session ID before a separate human Crew grant; resume requires the exact ID and unsupported local flags are refused. `cli.rs` routing and daemon-client transport are implemented, formatted and independently reviewed. The shared-conversation adapter is now applied in uncommitted source. Independent review corrections cover camelCase approvals, explicit provider precedence over defaults, terminal Error/Finish handling and streamed text/quiet output. Optional provider fields on `/agent/start` form an atomic pair and are validated/constructed before creating the session row. The shared GUI/CLI `/action-required/elicitation` endpoint requires Proven human authority, an exact session, an ordinary elicitation type and bounded data; durable answer/history receipts are implemented and independently reviewed. The exact-session ordinary waiter is claimed once before asynchronous persistence; the adopted-UID answer message and `MessagesPersisted` event precede live delivery. Cancellation records a user-only receipt. Typed outcomes distinguish 409 `unknown` (no write), 409 `recorded_not_delivered` (persisted, waiter ended) and 500 `persistence_failed`. Only typed 409 `unknown` is a CLI no-op; other failures surface without automatic resubmission. Focused passes cover daemon client 16 (before the later diagnostic display change), shared CLI options 4, shared conversation 5, provider binding 11, elicitation 4, UI form 2 and user-action store 6 (separate scopes, not summed). The strengthened elicitation suite covers persisted answer ID/body/agent-only visibility, Message then MessagesPersisted ordering, foreign-session/replay no-extra-row refusal, user-only cancellation with no answer, and persistence failure with no delivery or success events. UI typecheck passes after the response-object narrowing correction; Rust formatting and diff checks are clean. OpenAPI is regenerated including the 500 response. The final full gate, new matching native pair and shared-conversation live acceptance remain pending. Fresh fixture vault/device/save steps pass. Copying the verified broker from its incorrect system install location to the required user-local location restored authentication/connect; three-user bootstrap and a human roundtrip pass on pinned `4a2e190b`; the fixture awaits the new pair. This is fixture setup recovery, not new runtime acceptance. Safe SSH-stage diagnostics are applied in uncommitted source with allowlisted attach/handoff codes and static broker-install/identity guidance. Its selected diagnostic test passes (1); the encompassing command was interrupted during unrelated zero-match traversal, so command exit 0 is not claimed. These changes are not covered by the pinned `4a2e190b` checks or Linux validation, and do not establish full parity. Default local conversation mode remains separate. The contract requires identity-before-proof SSE, strict bounded parsing without automatic resubmission, attach-only `from_seq`, exact-turn cancellation and explicit typed approvals. Human proof is not forwarded to local workers.

Current `4a2e190b` observer slow-reader execution was refused at daemon identity verification before proof with environment EPERM; it remains unrun and no bypass was attempted. Fresh test-lane Docker/socket readiness passed and immutable `4a2e190b` Linux pair/filesystem 8/transfer 11/proof 3 checks pass; the verified production-pair UID 1101 lifecycle rerun passes. The earlier test-feature daemon smoke is explicitly unqualified; see [artifact provenance](validation-report.md#verified-pair-uid-1101-lifecycle-rerun). None validates the uncommitted conversation adapter.
...
| Ordinary terminal conversations | Reuse daemon agent/reply/session services and scoped grants; never forward human proof to local workers | Confirmed local-Agent manager split blocks ordinary run/session Crew MCP parity; explicit shared-daemon routing/client source is reviewed; the uncommitted conversation adapter is applied with review corrections; durable elicitation receipts are implemented/reviewed; focused tests are partial and runtime acceptance remains pending. |
