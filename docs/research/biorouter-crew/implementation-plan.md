...
for this plan. Current production HEAD is `9a3957d6`, after transfer checkpoint `67a32a07`; earlier validated artifacts are
...
Current production HEAD is `9a3957d6`, containing transport retirement and nine regression tests, after transfer/GUI/API checkpoint `67a32a07` (14 files) and documentation `808e891f`. Transfer source review is closed. Focused passes for committed transfer source are transfer authority 6, transfer unit 11, route proof 3 and filesystem 9, plus strict server Clippy, API generation and formatting. The actual GUI-handler regression passes seven tests with ESLint and Prettier. These scopes overlap and are not aggregated. The independently reviewed transport correction is committed in `9a3957d6`; nine focused transport regressions, strict core Clippy and formatting pass. Live confirmation, new builds and the full gate remain pending; the original broken-pipe root cause is still unresolved. No rebuilt runtime, complete gate, mixed-GUI or native Windows qualification is claimed for this checkpoint; manager-backed live changed-target confirmation remains pending.
...
| Local file capabilities and transfers | Narrow human-selected source/destination authority, bounded binary streaming, durable metadata receipts and atomic verified publication | Earlier source reviews closed for their recorded scope; the two P2 overwrite/partial-replacement corrections are committed in `67a32a07`, independently reviewed and covered by bounded focused tests; live replay remains open. Focused guards, 43-byte cross-user transfer and corrected 32 MiB restart/resume retain their bounded passes. Broader overwrite/path-replacement, resource/fault/reselection, graphical and Windows runtime matrices remain open. |
