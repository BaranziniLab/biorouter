...
Resumed scope, 2026-09-23: the Crew GUI redesign and human-readable identity requirements are acceptance requirements in [§16](#16-resumed-scope-gui-redesign-and-human-readable-identity-2026-09-23). The close-out state, deferred items and standing instructions are in the [resume handoff](handoff-2026-09-24.md#close-out-handoff-2026-09-25).
...
Status: Implemented; acceptance ran, with the results and the deferred items in [Final status](#final-status-2026-09-25). The user added these requirements when work resumed on 2026-09-23 (US Pacific; UTC 2026-09-24). They are acceptance requirements. Their designs were accepted on 2026-09-23: the [UI redesign specification](ui-redesign-spec.md) and the [naming design](naming-design.md) (decisions D1–D17, slices S0–S4). Implementation follows the [work packages](#work-packages-and-order) below. By 2026-09-24 all 27 implementation packages had reported and passed review, and an integrate-verify pass had fixed the defects it triaged on the integrated tree ([outcome](#integrate-verify-outcome-2026-09-24), [defect table and deferred items](handoff-2026-09-24.md#integrate-verify-pass-2026-09-24)). LIVE-QA, the 28th package, then ran four live QA rounds with fresh novice critics on a disposable AWS fixture, each followed by fix groups with independent review, and the close-out ran seven evidence-grade acceptance lanes on the final build `461f7899` ([evidence](evidence/ui-redesign-acceptance-2026-09.md)). Each package's outcome is recorded after the table, and results land in the [status ledger](implementation-status.md) under their own scopes. A design is not evidence. The close-out handoff and the dated decisions log are in the [resume handoff](handoff-2026-09-24.md#close-out-handoff-2026-09-25).
...
**Implemented.** All 28 packages are done, the round-1 to round-4 fix groups are committed with independent review (adversarial for the security-sensitive groups), and `join-by-name` is on by default. The seven final acceptance lanes ran on `461f7899`. A final polish then fixed their findings in 15 commits, ending at **`dc2746551fb422f271075e7291c876d97b53c153`**, and re-checked the fixes live there. Six last fixes followed, ending at the final code head **`715c59b5df6832c130d974a330d6db31cc5a7b2c`**, with no live run. The artifacts, the four live QA rounds, the final lanes and the final polish are in the [redesign acceptance evidence](evidence/ui-redesign-acceptance-2026-09.md). Against the criteria above:
...
- **GUI criterion 7 (presentation only).** Where a lane bypassed a disabled control from page script, the daemon refused (the institution lane's 400 `institution_refusal`), and every human-proof check refused a missing or wrong proof whatever the door.
...
**The last fixes (2026-09-25).** Six commits after `dc274655` fix NEW-2, NEW-3 and NEW-4 and two of the re-check's observations in the renderer. They also give the daemon its own F1 hold: Edit in place, `/reply`'s history write-back and diverge are refused with a 403, before anything stored changes, on a chat whose Crew access has ended. The local gate was ALL_GREEN on `715c59b5d`. They are verified by regression tests only; **no live fixture ran** ([evidence](evidence/ui-redesign-acceptance-2026-09.md#last-fixes-2026-09-25)).

**Deferred, with reasons:**

- **A registration proof only the main process holds** (the structural half of Q3-01 and Q4-55). Page script holds the daemon secret and the user-action proof, so it reaches `POST /crew/files` in both directions and also `POST /agent/call_tool`, and the CLI registers files with the same proof. Closing it needs a security design decision. The credential floor, the added stores and the download-destination rules ship as defense in depth.
- **Identity at scale**: the server-account name as secondary text for members who have not chosen a name (D2), and avatar hues by join order (D-AVATAR). Coordinator decisions.
- **Broker protocol additions**: editing or deleting one's own message, `blob.discard` for an uploaded-but-unsent file, and listing or cancelling legacy `{uid, public_key}` enrollments. Each needs a new broker method.
- **New features**: channel discovery for non-members (it would expose channel names; needs a spec), a status indicator outside Crew, a ⌘K switcher, @-mention suggestions, Reply, keyboard shortcuts and reflow at 400%.
