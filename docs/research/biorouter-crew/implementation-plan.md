...
for this plan. Current committed source is `a11ea7e9`; prior production artifacts are `4a2e190b`, after transfer checkpoint `67a32a07`; earlier validated artifacts are
...
| Ordinary terminal conversations | Reuse daemon agent/reply/session services and scoped grants; never forward human proof to local workers | Confirmed local-Agent manager split blocks ordinary run/session Crew MCP parity; explicit shared-daemon routing/client source is reviewed; the conversation adapter committed in `21ce28a2` is applied with review corrections; durable elicitation receipts are implemented/reviewed; focused tests are partial and runtime acceptance remains pending. |
