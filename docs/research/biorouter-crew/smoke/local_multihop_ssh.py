...
def main() -> int:
    ...
    try:
        ...
        ports = {"entry": 57101, "gate_a": 57102, "gate_b": 57103, "target": 57104}
        ...
        common = [
            "Host entry gate-a gate-b target",
            f"  User {user}",
        ...
        config.write_text("\n".join(common) + "\n")
        direct = run(["ssh", "-F", str(config), "target", "id -u; uname -srm"])
        ...
        if direct.returncode or multi.returncode or not result["changed_host_key"]["rejected"] or not result["cancelled_unreachable"]["side_effect_free"] or not result["cancelled_active"]["side_effect_free"]:
            return 1
        return 0
    finally:
        for pid in pids:
